...
for ($x = 1; $x <= $num; $x++) {
    ...
    if ($getID == 0) {
        ...
        $updateimage = mysqli_query($connection, "INSERT INTO paymethod_image SET paymeth_id = '" . trim($_REQUEST['txtproductimageID']) . "', image = '" . $imagefilename . "', imagename = '" . $Filename . "', date_added = '" . date("Y-m-d") . "';");
    } else {
        if ($prodname != "") {
            mkdir("../../OmaangatImages/Paymethod/", 0777, true);
            move_uploaded_file($prodname2, $Target_file);
