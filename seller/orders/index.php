                ...
                <table data-height="350" class="table table-bordered fixTable table-hover" style="margin-bottom: 0px;">
                    <thead class="bg-success text-white">
                        <tr>
                            ...
                            <th style="width: 15%;white-space: nowrap;"> Quantity </th>
...
<div class="d-flex justify-content-left mb-3 ml-3">
    <button class="btn mr-2" style="background: #4C644B; color: #F8FFFA;" onclick="exportToPDF()">Export Sale by
        Month</button>
    <button class="btn " style="border:1px solid  #4C644B; color: #4C644B;" onclick="exportToPDFbyWeekly()">Export Sale
        by Week</button>
</div>
...
<script>
    function exportToPDF() {
        ...
        for (var i = 0; i < tableSales.rows.length; i++) {
            ...
            if (orderStatus === 'Completed') {
                var salesAmount = parseFloat(tableSales.rows[i].cells[tableSales.rows[i].cells.length - 4].innerText
                    .replace(/,/g, ''));
    ...
    function exportToPDFbyWeekly() {
        ...
        for (var i = 0; i < tableSales.rows.length; i++) {
            var orderDateStr = tableSales.rows[i].cells[tableSales.rows[i].cells.length - 5]
                .innerText; // Assuming the date is in the 5th column
            ...
            if (weekNum >= 0 && weekNum < weekNames.length) {
                totalSalesPerWeek[weekNum] += Number(tableSales.rows[i].cells[tableSales.rows[i].cells.length - 3]
                    .innerText); // Assuming the sales amount is in the last column
                if (tableSales.rows[i].cells[tableSales.rows[i].cells.length - 4].innerText ===
                    "Completed") { // Assuming the status is in the third column
