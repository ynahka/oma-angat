<script type="text/javascript">
    ...
    function toshipapprove(order_id) {
        ...
        // Check if courier and tracking number are already present in the database
        if (existingCourier !== '' && existingTrackingNumber !== '') {
            // If present, approve pending and topay before completing order
            $.ajax({
                type: 'POST',
                url: 'orders/class.php',
                data: 'order_id=' + order_id + '&form=pendingapprove',
                success: function(data) {
                    // After pending approval, proceed to topay approval
                    $.ajax({
                        type: 'POST',
                        url: 'orders/class.php',
                        data: 'order_id=' + order_id + '&form=topayapprove',
                        success: function(data) {
                            // After topay approval, proceed to complete order
                            $.ajax({
                                type: 'POST',
                                url: 'orders/class.php',
                                data: 'order_id=' + order_id + '&form=toshipapprove',
                                success: function(data) {
                                    setTimeout(function() {
                                        $(".preloader").hide().css('background',
                                            '');
            ...
            // If not present, proceed with the existing logic for "toshipapprove"
            var courier = $('.courier').val();
            var trackingnumber = $('.trackingnumber').val();
