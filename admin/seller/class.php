...
switch ($_POST['form']) {
	...
	case "loadcustomerlistPagination":
		...
		$page = $_POST["page"];
		$rowCount = mysqli_fetch_row(mysqli_query($connection, "SELECT COUNT(a.id) FROM users_table AS a LEFT JOIN user_details AS b ON a.user_id = b.user_id WHERE a.usertype = 'SELLER' AND a.status = 'APPROVED' " . $searchuseracc . ";"));
		$rowsperpage = 10;
		$range = 1;
	...
	case 'fncdsplycustinfo':
		...
		if ($custdet[4] == "") {
			...
			$img = "../OmaangatImages/ValidIDs/" . $custdet[4];
		}

		if ($custdet[9] == "") {
			$img2 = "assets/images/noimage5.png";
		} else {
			$img2 = "../OmaangatImages/ValidIDs/" . $custdet[9];
		}

		echo $custdet[0] . "|" . $custdet[1] . "|" . $custdet[2] . "|" . $custdet[3] . "|" . $img . "|" . $custdet[5] . "|" . $custdet[6] . "|" . $custdet[7] . "|" . $custdet[8] . "|" . $img2;
		break;
	...
	case 'displaymdlappsellerlist':
		...
		$page = $_POST["page"];
		$limit = ($page - 1) * 10;
		$res = mysqli_query($connection, "SELECT a.user_id, CASE WHEN a.middlename = '' OR a.middlename IS NULL THEN CONCAT(a.lastname, ', ', a.firstname) ELSE CONCAT(a.lastname, ', ', a.firstname, ' ', LEFT(a.middlename, '1'), '.') END, b.contactnum, b.address, a.status, b.image, a.id FROM users_table AS a LEFT JOIN user_details AS b ON a.user_id = b.user_id WHERE a.usertype = 'SELLER' AND a.status = 'PENDING' ORDER BY a.user_id ASC LIMIT " . $limit . ",10");
		$numrows = mysqli_num_rows($res);
		if ($numrows == TRUE) {
			while ($row = mysqli_fetch_array($res)) {
				...
				echo "<tr style='cursor:pointer;'>
	                        ...
	                        <td style='white-space: nowrap;'>" . $row[2] . "</td>
	                        <td style='white-space: nowrap; text-align: center;'>
	                        	<button type='button' class='btn waves-effect waves-light btn-outline-success' onclick='approveseller(\"" . $row[6] . "\")'>Approve</button>
	                        </td>
	                        <td style='white-space: nowrap; text-align: center;'>
	                        	<i class='fas fa-eye fa-lg text-info' style='cursor:pointer;' onclick='modalopencustprof(\"" . $row[6] . "\")' title='View Info'></i>
	                        </td>
	                    </tr>";
			}
		} else {
	...
	case "loadmdlappselllistPagination":
		$page = $_POST["page"];
		$rowCount = mysqli_fetch_row(mysqli_query($connection, "SELECT COUNT(a.id) FROM users_table AS a LEFT JOIN user_details AS b ON a.user_id = b.user_id WHERE a.usertype = 'SELLER' AND a.status = 'PENDING';"));
		$rowsperpage = 10;
		$range = 1;
		$totalpages = ceil($rowCount[0] / $rowsperpage);
		$prevpage;
	...
	case 'approveseller':
		$approveseller = mysqli_query($connection, "UPDATE users_table SET status = 'APPROVED' WHERE id = '" . $_POST['ID'] . "'");
		break;
}
