<?php
	...
	switch ($_POST['form']) {
		...
		case "loadorderslistPagination":
			...
			$page = $_POST["page"];
			$rowCount = mysqli_fetch_row(mysqli_query($connection, "SELECT COUNT(a.id) FROM payments AS a LEFT JOIN orders AS b ON a.order_id = b.order_id LEFT JOIN users_table AS c ON b.customer_id = c.user_id LEFT JOIN products AS d ON b.product_id = d.product_id WHERE a.id != '' " . $searchuseracc . " " . $orderstat . " " . $DateFilter . ";"));
			$rowsperpage = 10;
			$range = 1;
		...
		case 'fncdsplyorderinfo':
            ...
            $proddetails = mysqli_fetch_array(mysqli_query($connection, "SELECT productname, commonname, latinname, type, familyname FROM products WHERE product_id = '" . $_POST['product_id'] . "';"));
