...
switch ($_POST['form']) {
	...
	case 'fncdsplylistofcart':
		$count = 0;
		$res = mysqli_query($connection, "SELECT order_id, product_id, quantity, price, totalamt, orderstatus FROM orders WHERE customer_id = '" . $_SESSION['user_id']  . "';");
		...
		if ($numrows == TRUE) {
			while ($row = mysqli_fetch_array($res)) {
				...
				echo "<tr>
                            ...
                            <td style='min-width:2px'>
                                <a href='javascript:void(0)' class='tablebuttonneed' onclick='openproductbuynowdet(\"" . $count . "\", \"" . $row[1] . "\", \"" . $row[0] . "\")'>View</a>
	...
	case 'fncdisplaybuynowproddet':
		...
		// ORDER DETAIL
		$getorderdet = mysqli_fetch_array(mysqli_query($connection, "SELECT quantity, price, shipfee, totalamt, paymenttype, orderstatus, orderstatus2, courier, trackingnumber FROM orders WHERE order_id = '" . $_POST['order_id'] . "';"));
	...
	case 'deletecart':
		$deletecart = mysqli_query($connection, "DELETE FROM orders WHERE order_id = '" . $_POST['order_id'] . "'");
		break;
}
