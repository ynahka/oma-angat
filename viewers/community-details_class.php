...
switch ($_POST['form']) {
        ...
        // 	$res = mysqli_query($connection, "SELECT post_id, user_id, posttitle, postdescription, image, imagename, date_added FROM post WHERE post_id = '" . $_POST['textpost_id']  . "'");
        // 	$numrows = mysqli_num_rows($res);
        // 	if($numrows == TRUE){
        // 		while($row = mysqli_fetch_array($res)){

        // 			$getuser = mysqli_fetch_array(mysqli_query($connection, "SELECT CASE WHEN middlename = '' OR middlename IS NULL THEN CONCAT(lastname, ', ', firstname) ELSE CONCAT(lastname, ', ', firstname, ' ', LEFT(middlename, '1'), '.') END FROM usesrs_table WHERE user_id = '" . $row[1] . "';"));
